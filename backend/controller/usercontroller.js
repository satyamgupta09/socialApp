...
const fs = require('fs');
cloudinary.config({
    cloud_name: 'djuccf8wu',  // replace with your cloud name
    api_key: '272729648258675',        // replace with your API key
    api_secret: '3J3V7-OscMrBtRcMY-lR72UR5as'   // replace with your API secret
});
...
const getSuggestions = async (req, res) => {
    try {
        ...
        const userAllFriends = await User.findById(req.userData.userData.userId).populate('friend');
        const excludeIds = [...userAllFriends.friend.map(f => f._id), req.userData.userData.userId];

        // ✅ Get total count first
        const totalUsers = await User.countDocuments({ _id: { $nin: excludeIds } });

        // ✅ If no more users to paginate
        if ((page - 1) * perPage >= totalUsers) {
...
const uploadDp = async (req, res) => {
    ...
    User.uploadedAvatar(req, res, async function (error) {
        ...
        if (req.file) {
            ...
            fs.unlinkSync(req.file.path); // optional: remove local file
...
const uploadCoverPhoto = async (req, res) => {
    ...
    User.uploadedCoverPhoto(req, res, async function (error) {
        if (error) {
        ...
        if (req.file) {
            ...
            // for cloudinery
            const result = await cloudinary.uploader.upload(req.file.path, { folder: 'cover_photos' });
...
const uploadPost = async (req, res) => {
    ...
    User.uploadedAvatar(req, res, async function (error) {
        ...
        try {
            ...
            // for cloudinery
            const result = await cloudinary.uploader.upload(req.file.path, { folder: 'post_images' });
